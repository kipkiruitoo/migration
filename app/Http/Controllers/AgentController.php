...
class AgentController extends Controller
{
    ...
    public function thirdpage(Request $request)
    {
        ...
        if (isset($request->search)) {
            ...
            $respondent = Respondent::where('phone', 'like',  $request->phone)->where('project', $request->project)->get();
    ...
    public function addfeedback(Request $request)
    {
        ...
        $respondent = Respondent::where('id', $request->input('respondent'))->first();

        // dd($request->input('feedback'));

        if ($request->input('feedback') == 'Not Interested' or $request->input('feedback') == 'Inconsistent Respondent Data' or $request->input('feedback') == 'Already Participated') {
            $respondent->status = 'Inactive';
    ...
    public function fetchuser($project, $daysbetweeninterviews)
    {
        ...
        if (!$respondents) {
            ...
            } else if (Interview::where('project', $project)->where('phonenumber', $respondent[0]->phone)->exists()) {
                return
                    $this->fetchuser($project, $daysbetweeninterviews);
            }
