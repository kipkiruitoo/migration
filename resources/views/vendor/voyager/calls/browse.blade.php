...
@section('page_header')
    <div class="container-fluid">
        ...
        @can('edit', app($dataType->model_name))
            ...
            @endif
        @endcan
            {{-- <button  data-toggle="modal" data-target="#export" class="btn  btn-dark"><i class="voyager-download"></i><span> Export</span></button> --}}
        @can('delete', app($dataType->model_name))
            @if($usesSoftDeletes)
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            ...
                                            <td>
                                                ...
                                                @elseif($row->type == 'file' && !empty($data->{$row->field}) )
                                                    ...
                                                    @else
                                                    @php
                                                                 $link_array = explode('/',$data->{$row->field});
                                                                 $newlink = 'https://voip.tifaresearch.com/' . end($link_array)
                                                            @endphp
                                                        <audio controls>
                                                            {{-- <source src="horse.ogg" type="audio/ogg"> --}}
                                                            <source src="{{ $newlink }}" type="audio/mpeg">
                                                            Your browser does not support the audio element.
                                                        </audio>
                                                        <a class="btn btn-primary" style="margin-left: 10px; margin-bottom: 10px" href="{{ $newlink }}" target="_blank">
                                                            Download
                                                        </a>
                                                    @endif
...
<div class="modal fade" id="export" tabindex="-1" role="dialog"
                    aria-labelledby="export" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <form action="{{route('feedbackexports')}}" method="POST">
                             @csrf
